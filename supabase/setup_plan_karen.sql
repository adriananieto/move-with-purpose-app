-- Karen J Rodriguez Navarro's training plan, from
-- "MOVE WITH PURPOSE - KAREN (14septiembre).pdf" — 5 días de fuerza
-- (Lower/Upper/Lower/Upper/Lower), sin instrucciones de chat que lo cambien,
-- así que se usa tal cual el PDF (no el split de Gixallys).
-- Condición: Hipotiroidismo — sin restricción de ejercicios indicada.
--
-- Run create_client_karen.sql FIRST. Safe to re-run.

drop table if exists _karen;
create temporary table _karen as
select p.id as client_id
from public.profiles p
join auth.users u on u.id = p.id
where u.email = 'karen_rodriguez_13@hotmail.com';

do $$
begin
  if (select count(*) from _karen) = 0 then
    raise exception 'No profile matched that email. Run create_client_karen.sql first, or check the address.';
  end if;
end $$;
...
where not exists (
  select 1 from public.workouts w where w.client_id = _karen.client_id and w.name = v.name
);
...
join public.workouts w on w.client_id = (select client_id from _karen) and w.name = x.day_name
...
where user_id = (select client_id from _karen);
...
where w.client_id = (select client_id from _karen)
