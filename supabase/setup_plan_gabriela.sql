-- Gabriela De Jesus Alvarado's training plan — exact same plan as Karen
-- J Rodriguez Navarro, per Adriana's instruction.
--
-- Run create_client_gabriela.sql FIRST. Safe to re-run.

drop table if exists _gabriela;
create temporary table _gabriela as
select p.id as client_id
from public.profiles p
join auth.users u on u.id = p.id
where u.email = 'dejesusgabriella76@gmail.com';

do $$
begin
  if (select count(*) from _gabriela) = 0 then
    raise exception 'No profile matched that email. Run create_client_gabriela.sql first, or check the address.';
  end if;
end $$;
...
where not exists (
  select 1 from public.workouts w where w.client_id = _gabriela.client_id and w.name = v.name
);
...
join public.workouts w on w.client_id = (select client_id from _gabriela) and w.name = x.day_name
...
where user_id = (select client_id from _gabriela);
...
where w.client_id = (select client_id from _gabriela)
