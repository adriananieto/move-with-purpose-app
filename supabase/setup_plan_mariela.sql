-- Mariela Nieto's training plan — exact same plan as Karen J Rodriguez Navarro,
-- per Adriana's instruction.
--
-- Run create_client_mariela.sql FIRST. Safe to re-run.
...
where not exists (
  select 1 from public.workouts w where w.client_id = _mariela.client_id and w.name = v.name
);
...
join public.workouts w on w.client_id = (select client_id from _mariela) and w.name = x.day_name
...
where w.client_id = (select client_id from _mariela)
